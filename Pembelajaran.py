...
def app():
    ...
    with tabs[0]:
        # Intro
        #APAKAH FISIKA ITU?
        st.title(':green[APAKAH FISIKA ITU?]')
        st.write('Fisika fluida adalah dasar dari teknik hidrolik, sebuah cabang teknik yang diaplikasikan di banyak bidang. Seorang insinyur nuklir mungkin mempelajari aliran fluida dalam sistem hidrolik reaktor nuklir yang sudah tua, sementara seorang insinyur medis mungkin mempelajari aliran darah dalam arteri pasien yang sudah tua. Seorang insinyur lingkungan mungkin peduli dengan drainase dari tempat pembuangan sampah atau irigasi yang efisien untuk lahan pertanian. Seorang insinyur angkatan laut mungkin peduli dengan bahaya yang dihadapi oleh penyelam laut dalam atau dengan kemungkinan kru melarikan diri dari kapal selam yang jatuh. Seorang insinyur aeronautika mungkin merancang sistem hidrolik yang mengendalikan flap sayap yang memungkinkan pesawat jet mendarat. Rekayasa hidrolik juga diterapkan di banyak pertunjukan Broadway dan Las Vegas, di mana set yang sangat besar dengan cepat dipasang dan diturunkan dengan sistem hidrolik. Sebelum kita dapat mempelajari aplikasi fisika fluida, kita harus terlebih dahulu menjawab pertanyaan “Apa itu fluida?”')
        
        st.title(':orange[1.1 APAKAH ITU FLUIDA?]')
        st.write('Fluida memainkan peran penting dalam banyak aspek kehidupan sehari-hari. Kita meminumnya, menghirupnya, berenang di dalamnya. Fluida bersirkulasi di seluruh tubuh kita dan mengendalikan cuaca. Oleh karena itu, fisika fluida sangat penting bagi pemahaman kita tentang alam dan teknologi. Kami memulai studi kami dengan statika fluida, studi tentang fluida yang diam dalam situasi kesetimbangan. Seperti situasi kesetimbangan lainnya, ini didasarkan pada hukum pertama dan ketiga Newton. Kita akan mengeksplorasi konsep-konsep kunci dari densitas, tekanan, dan daya apung. Meskipun begitu, kita hanya akan membahas permukaan dari topik yang luas dan menarik ini')
        st.write('Fluida adalah zat apa pun yang dapat mengalir dan mengubah bentuk volume yang ditempatinya. (Sebaliknya, benda padat cenderung mempertahankan bentuknya.) Kami menggunakan istilah “fluida” untuk gas dan cairan. Perbedaan utama di antara keduanya adalah cairan memiliki kohesi, sedangkan gas tidak. Molekul-molekul dalam cairan berdekatan satu sama lain, sehingga mereka dapat mengerahkan gaya tarik-menarik satu sama lain dan dengan demikian cenderung untuk tetap bersama (yaitu, menyatu). Itulah mengapa sejumlah cairan mempertahankan volume yang sama saat mengalir. Contohnya: Jika Anda menuangkan 500 mL air ke dalam panci, air akan tetap menempati volume 500 mL. Sebaliknya, molekul-molekul gas, dipisahkan rata-rata dengan jarak yang jauh lebih besar dari ukuran molekul. Oleh karena itu kekuatan antar molekul lemah, hanya ada sedikit atau tidak ada kohesi, dan gas dapat dengan mudah perubahan volume. Jika Anda membuka katup pada tangki oksigen terkompresi yang memiliki volume 500 mL, oksigen akan mengembang ke volume yang jauh lebih besar.')
        
        #1.1 KEPADATAN DAN BERAT JENIS
        st.title(':orange[1.1 KEPADATAN DAN BERAT JENIS]')
        ...
        st.write('Massa jenis, $ρ$, dari suatu zat ($ρ$ adalah huruf kecil Yunani rho) didefinisikan sebagai massa per satuan volume:')        
        ...
        st.write('di mana m adalah massa sampel zat dan V volumenya. Massa jenis adalah sifat karakteristik dari zat murni apa pun. Benda yang terbuat dari zat murni tertentu, seperti emas murni, dapat memiliki ukuran atau massa berapa pun, tetapi massa jenisnya akan sama untuk setiap benda.')
        st.write('Kita dapat menggunakan massa jenis, Persamaan 1-1, untuk menulis massa suatu benda sebagai')
        st.latex(r'm = {\rho}{V}')
        st.write('dan berat suatu benda adalah sebagai berikut')
        st.latex(r'{m}{g} = {\rho}{V}{g}')
        st.write('Satuan SI untuk densitas adalah $kg/m^3$. Kadang-kadang densitas diberikan dalam $g/cm^3$. Perhatikan bahwa densitas yang diberikan dalam $g/cm^3$ harus dikalikan dengan 1000 untuk memberikan hasil dalam kg/m^3. Contohnya, densitas aluminium adalah $ρ = 2.70 g/cm^3$, yang sama dengan $2700 kg/m^3$. Densitas berbagai zat diberikan dalam Tabel 1-1. Tabel tersebut menentukan suhu dan tekanan atmosfer karena keduanya mempengaruhi massa jenis (efeknya sedikit untuk cairan dan padatan). Perhatikan bahwa udara sekitar 1000 kali lebih tidak padat daripada air.')
        st.image('gambar/1.png', caption='Gambar 1. Massa Jenis Beberapa Zat Umum')
        st.write('Konsep densitas dan berat jenis sangat membantu dalam mempelajari fluida karena kita tidak selalu berurusan dengan volume atau massa yang tetap.')
        with st.expander("📚**Latihan Soal**", expanded=False):
            ...
            st.write('***EVALUASI***: Satu ruangan yang penuh dengan udara memiliki berat yang hampir sama dengan berat rata-rata orang dewasa. Air hampir seribu kali lebih padat daripada udara, sehingga massa dan beratnya massa dan beratnya lebih besar dengan faktor yang sama. Berat satu ruangan penuh air akan akan meruntuhkan lantai rumah biasa.')
